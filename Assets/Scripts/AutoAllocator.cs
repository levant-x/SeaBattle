...
public class AutoAllocator : GameField
{
    ...
    void AutoLocateShip(Ship ship)
    {
        ...
        for (int i = 0; i < body.Length; i++)
        {
            ...
            Debug.Log($"selected area {areaNum} {selectedArea.min} {selectedArea.max}");

            Debug.Log($"taken area {areaNum} {selectedArea.min} {selectedArea.max} ship floors {ship.FloorsNum()}");

            if (!CheckAndAdjustSpawnAreaAndOrient(ship, ref selectedArea))
                areasWorkingList.Remove(randomArea);
            else break;
        }
    ...
    bool CheckAndAdjustSpawnAreaAndOrient(Ship ship, ref Bounds area)
    {
        ...
        { ship.orientation = (Ship.Orientation)Random.Range(0, 2); Debug.Log("random orient " + ship.orientation); }
